...
import ac.mdiq.podvinci.net.common.UrlChecker.urlEquals
...
class UrlCheckerTest {
    ...
    @Test
    fun testPodVinciSubscribeProtocolNoScheme() {
        val inVal = "podvinci-subscribe://example.com"
        val out = prepareUrl(inVal)
        Assert.assertEquals("http://example.com", out)
    }
    ...
    @Test
    fun testPodVinciSubscribeProtocolWithScheme() {
        val inVal = "podvinci-subscribe://https://example.com"
        val out = prepareUrl(inVal)
        Assert.assertEquals("https://example.com", out)
    }
    ...
    fun testPodVinciSubscribeDeeplink() {
        val feed = "http://example.org/podcast.rss"
        Assert.assertEquals(feed, prepareUrl("https://podvinci.org/deeplink/subscribe?url=$feed"))
        Assert.assertEquals(feed, prepareUrl("http://podvinci.org/deeplink/subscribe?url=$feed"))
        Assert.assertEquals(feed, prepareUrl("http://podvinci.org/deeplink/subscribe/?url=$feed"))
        Assert.assertEquals(feed, prepareUrl("https://www.podvinci.org/deeplink/subscribe?url=$feed"))
        Assert.assertEquals(feed, prepareUrl("http://www.podvinci.org/deeplink/subscribe?url=$feed"))
        Assert.assertEquals(feed, prepareUrl("http://www.podvinci.org/deeplink/subscribe/?url=$feed"))
        Assert.assertEquals(feed, prepareUrl("http://www.podvinci.org/deeplink/subscribe?url="
    ...
    fun testProtocolRelativeUrlIsHttpsWithApSubscribeProtocol() {
        val inVal = "//example.com"
        val inBase = "podvinci-subscribe://https://examplebase.com"
        val out = prepareUrl(inVal, inBase)
        Assert.assertEquals("https://example.com", out)
    }
