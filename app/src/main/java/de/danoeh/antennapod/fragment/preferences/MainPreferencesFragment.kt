...
import de.danoeh.antennapod.activity.PreferenceActivity.Companion.getTitleOfPage
...
class MainPreferencesFragment : PreferenceFragmentCompat() {
    override fun onCreatePreferences(savedInstanceState: Bundle?, rootKey: String?) {
        ...
        if (packageHash != 1790437538 && packageHash != -1190467065) {
            ...
            copyrightNotice.summary = ("This application is based on AntennaPod."
            ...
            debugNotice.order = -1
            debugNotice.summary = "This is a development version of AntennaPod and not meant for daily use"
            findPreference<Preference>(PREF_CATEGORY_PROJECT)!!.parent!!
    ...
    private fun setupMainScreen() {
        ...
        findPreference<Preference>(PREF_VIEW_FORUM)!!.onPreferenceClickListener =
            Preference.OnPreferenceClickListener { preference: Preference? ->
                openInBrowser(requireContext(), "https://forum.antennapod.org/")
                true
            }
            ...
            try {
                requireContext().assets.open("website-languages.txt").use { `is` ->
                    ...
                    return if (ArrayUtils.contains(languages, deviceLanguage) && "en" != deviceLanguage) {
                        "https://antennapod.org/$deviceLanguage"
                    } else {
