...
import ac.mdiq.podvinci.core.util.comparator.DownloadResultComparator
import ac.mdiq.podvinci.core.util.comparator.PlaybackCompletionDateComparator
...
import ac.mdiq.podvinci.storage.database.mapper.*
import ac.mdiq.podvinci.storage.database.mapper.DownloadResultCursorMapper.convert
import ac.mdiq.podvinci.storage.preferences.UserPreferences
import ac.mdiq.podvinci.storage.preferences.UserPreferences.feedCounterSetting
import ac.mdiq.podvinci.storage.preferences.UserPreferences.feedOrder
import java.util.*
import kotlin.math.min


/**
 * Provides methods for reading data from the PodVinci database.
    ...
    private fun getImageAuthentication(imageUrl: String, adapter: PodDBAdapter?): String {
        ...
        adapter!!.getImageAuthenticationCursor(imageUrl).use { cursor ->
            if (cursor.moveToFirst()) {
                ...
                credentials = if (!TextUtils.isEmpty(username) && password != null) {
                    "$username:$password"
                } else {
                    ""
                }
...
//        }
    ...
    fun getNavDrawerData(subscriptionsFilter: SubscriptionsFilter?): NavDrawerData {
        ...
        val folders: MutableMap<String?, TagDrawerItem> = HashMap()
        for (feed in feeds) {
            for (tag in feed.preferences!!.getTags()) {
                ...
                var folder: TagDrawerItem?
                if (folders.containsKey(tag)) {
                    folder = folders[tag]
                } else {
                    folder = TagDrawerItem(tag)
                    folders[tag] = folder
                }
                drawerItem.id = drawerItem.id or folder!!.id
                folder.children.add(drawerItem)
            }
        }
        val foldersSorted = folders.values.toMutableList()
        foldersSorted.sortWith { o1: TagDrawerItem, o2: TagDrawerItem ->
